...
func (gps Groups) Aggregation(typs []AggregationType, colnames []string) DataFrame {
	...
	for k, v := range dfMaps[0] {
		...
		case bool:
			colTypes[k] = series.Bool
		default:
			colTypes[k] = series.String // 默认使用字符串类型
		}
	}
...
// Row returns a map[string]interface{} representing the row at the given index
func (df DataFrame) Row(index int) (map[string]series.Element, map[string]interface{}) {
	if df.Err != nil {
		return nil, nil
	}

	if index < 0 || index >= df.nrows {
		df.Err = fmt.Errorf("row index out of range: %d", index)
		return nil, nil
	}

	row := make(map[string]series.Element)
	rowData := make(map[string]interface{})
	for _, col := range df.columns {
		row[col.Name] = col.Elem(index)
		rowData[col.Name] = col.Val(index)
	}
...
// the same column names in the same order, and all elements in each column are equal.
func (df DataFrame) Equal(other DataFrame) bool {
	if df.Err != nil || other.Err != nil {
		return false
	}

	if df.nrows != other.nrows || df.ncols != other.ncols {
		return false
	}
...
// ValuesOption is a function type to set options
type ValuesOption func(*ValuesOptions)

// WithRowIndex sets the option to return row index
func WithRowIndex(r bool) ValuesOption {
	return func(o *ValuesOptions) {
		o.returnRowIndex = r
	}
}

// WithRowData sets the option to return row data
func WithRowData(r bool) ValuesOption {
	return func(o *ValuesOptions) {
		o.returnRowData = r
	}
}

// WithSelectedColumns sets the columns to be returned
func WithSelectedColumns(columns ...string) ValuesOption {
	return func(o *ValuesOptions) {
		o.selectedColumns = columns
	}
}
...
func (df DataFrame) RowsIterator(opts ...ValuesOption) func() (int, map[string]interface{}, bool) {
	...
	// Create a map of column indices for selected columns
	selectedColIndices := make(map[int]bool)
	if len(options.selectedColumns) > 0 {
		for _, colName := range options.selectedColumns {
			idx := df.colIndex(colName)
			if idx != -1 {
				selectedColIndices[idx] = true
			}
		}
	} else {
		// If no columns are specified, select all columns
		for i := 0; i < df.ncols; i++ {
			selectedColIndices[i] = true
		}
	}
	...
	return func() (int, map[string]interface{}, bool) {
		...
		if options.returnRowIndex {
			rowIndex = row
		} else {
			rowIndex = -1
		}

		if options.returnRowData {
			for j, col := range df.columns {
				if selectedColIndices[j] {
					rowData[col.Name] = col.Val(row)
				}
			}
		}

		row++
		return rowIndex, rowData, true
	}
}
